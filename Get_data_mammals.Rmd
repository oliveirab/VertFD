...
if(answer=="no"){
  load(paste(savedatafolder,"code/mammals",'_data.RData',sep=""))
  #load(paste(savedatafolder,"code/mammals",'_data_cluster.RData',sep=""))
}
if(answer=="yes"){
  
occr <- read.csv(paste(
  savedatafolder,"/Data/mammals_occr_equal_area_30land.csv",sep=""))
XY <- occr[,1:2]
names(XY) <- c("x","y")
coordinates(XY)<-~x+y
occr <- occr[,-1:-2]
...
### OPEN LARGE DATASET
trait <- fread(paste(sep="",savedatafolder,"Data/Elton_mammals.txt"))

names(trait) 
 
# Get traits of interest
trait <- data.frame(trait[,c(2,4:13,16,19:21,24)]) 
trait <- na.omit(trait)
...
## Phylogeny
Using Hedges et al.,2015
```{r open_traits, eval=F}
### OPEN LARGE DATASET
tre <- read.tree(paste(sep="",savedatafolder,"Data/Mammal_tree_Hegdes.nwk"))
...
```{r get_velocity, eval=F}
# Climate stability over time metric
...
"NA",
"Tundra",
"Mediterranean Forest/Shrublands",
"Desert",
"Mangroves")
...
## BioRealm following Jetz Fine 2012
```{r get_tree_density}
biome2 <- rep(NA,length(biome))

biome2[which(biome == 11)] <- "Tundra"
biome2[which(biome == 13)] <- "Desert"
biome2[which(biome == 8)] <- "Grassland"
biome2[which(biome == 6)] <- "BorealForest"
biome2[which(biome == 4 | biome == 5)] <- "TemperateForest"
biome2[which(biome == 1)] <- "TropicalMoistWetForest"
biome2[which(biome == 7 | biome == 2)] <- "TropicalDryForestSavanna"
biome2[which(biome == 12)] <- "MediterraneanForestShrublands"

Biorealm <- paste(biome2, Realm, sep = "_")

test <- plot(rasterFromXYZ(data.frame(XY,as.numeric.factor(as.factor(biome2)))))
```
...
env.vars <- data.frame(velocity,climate.stab,temp.stab,tempnov,tempdist,precipdist,precipnov,climdist,climnov,
                       ...
                       Realm,Biorealm,biome=biome2)
```

# COMMUNITY METRICS
```{r community_metrics, eval=F, message=FALSE}
trc <- read.tree(paste(sep="",savedatafolder,"Data/Mammal_tree_Hegdes.nwk"))
...
tabN <- data.frame(apply(trait[,c(11:14)], 2, as.factor))
# Now with the quantitative Variables
tabQ <- data.frame(trait[,c(1:10,15)])
...
# WRITE RESULTS
```{r}
write.csv(metadata, paste(savedatafolder,"Results/mammals",'_metadata.csv',sep=""))
```

# SAVE
```{r echo=F, eval=FALSE}
save.image(paste(savedatafolder,"Code/mammals",'_data.RData',sep=""))
```
...
biome2 <- cbind(teste2, ifelse(teste[,-1]>0, 1, 0))
biome3 <- biome2[,1]
teste3 <- biome2[,-1]
rownames(teste3) <- biome3

# Does at least one community has zero species?
any(apply(teste3, 1, sum)==0) 

occr2 <- as.data.frame(teste3) ; rm(teste3, teste2, teste)
```